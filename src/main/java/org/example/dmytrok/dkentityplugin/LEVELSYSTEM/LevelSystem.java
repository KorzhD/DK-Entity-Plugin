...
public class LevelSystem {
    ...
    public static void addExp(Player player, int amount) {
        ...
        if (currentExp >= requiredExp) {
            ...
            ScoreboardManager.updateScore(player, "level", "§l§dLevel: " + currentLevel);
            ScoreboardManager.updateScore(player, "newExp", "§6To new Level: " + getRequiredExp(getLevel(player)));
        }
        playerExp.put(uuid, currentExp);
        ScoreboardManager.updateScore(player, "exp", "§l§6Exp: " + currentExp);
    }
